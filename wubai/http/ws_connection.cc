...
std::pair<HttpResult::ptr, WSConnection::ptr> WSConnection::Create(const std::string& url,
    ...
    if(!uri) {
        return std::make_pair(std::make_shared<HttpResult>((int)HttpResult::Error::INVALID_HOST, nullptr, "invalid url:" + url), nullptr);
    }
...
std::pair<HttpResult::ptr, WSConnection::ptr> WSConnection::Create(Uri::ptr uri, 
    ...
    if(!addr) {
        return std::make_pair(std::make_shared<HttpResult>((int)HttpResult::Error::INVALID_HOST, nullptr, "invalid url:" + uri->getHost()), nullptr);
    }
    Socket::ptr sock = Socket::CreateTCP(addr);
    if(!sock) {
        return std::make_pair(std::make_shared<HttpResult>((int)HttpResult::Error::CREATE_SOCKET_FAIL
    ...
    if(!has_conn) {
        req->setHeaders("Connection", "Upgrade");
    }
    req->setHeaders("Upgrade", "websocket");
    req->setHeaders("Sec-webSocket-Version", "13");
    req->setHeaders("Sec-webSocket-Key", wubai::base64encode(random_string(16)));
    if(!has_host) {
