...
IPAddress::ptr IPAddress::Create(const char* address, uint16_t port) {
    ...
    if(error) {
        WUBAI_LOG_ERROR(g_logger) << "IPAddress::Create(" << address << ", " << port << ") error = " << error << " errno = " << errno << " errstr" << strerror(errno);
        return nullptr;
    }
    try {
...
bool Address::Lookup(std::vector<Address::ptr>& result, const std::string& host, int family, int type,int protocol) {
    ...
    if(node.empty()) {
        ...
        if(service) {
            if(!memchr(service + 1, ':', host.c_str() - service + host.size() - 1)) {
    ...
    if(error) {
        WUBAI_LOG_ERROR(g_logger) << "Address::Lookup getaddress ( " << node << " , " << family << " , " << type << ") err= " << error << "errstr = " << strerror(errno);
        return false;
    }
...
bool Address::GetInterfaceAddresses(std::multimap<std::string, std::pair<Address::ptr, uint32_t> >& result, int family) {
    ...
    if(getifaddrs(&results) != 0) {
        WUBAI_LOG_ERROR(g_logger) << "Address::GetInterfaceAddresses getifaddrs" << "err=" << errno << "errorstr = " << strerror(errno);
        return false;
    }
    try {
        for(next = results; next; next = next->ifa_next) {
            ...
            switch(next->ifa_addr->sa_family) {
                case AF_INET:
                    {
                        address = Create(next->ifa_addr, sizeof(next->ifa_addr));
        ...
        WUBAI_LOG_ERROR(g_logger) << "Address::GetInterfaceAddresses exception";
        freeifaddrs(results);
        return false;
    }
    freeifaddrs(results);
...
IPv4Address::ptr IPv4Address::Create(const char* address, uint16_t port) {
    ...
    if(result <= 0) {
        WUBAI_LOG_ERROR(g_logger) << "IPv4Address::Create(" << address << ", " << port << ") rt=" << result << "errno = " << errno << "errstr = " << strerror(errno);
        return nullptr;
    }
    return rt;
}
...
IPv6Address::ptr IPv6Address::Create(const char* address, uint16_t port) {
    ...
    if(result <= 0) {
        WUBAI_LOG_ERROR(g_logger) << "IPv6Address::Create(" << address << ", " << port << ") rt=" << result << "errno = " << errno << "errstr = " << strerror(errno);
        return nullptr;
    }
    return rt;
}
...
IPAddress::ptr IPv6Address::subnetMask(uint32_t prefix_len) {
    ...
    for(uint32_t i = prefix_len / 8 + 1; i < prefix_len / 8; ++i) {
