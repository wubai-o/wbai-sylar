...
static void ListAllMember(const std::string& prefix, const YAML::Node& node, std::list<std::pair<std::string,const YAML::Node> >& output) {
    if(prefix.find_first_not_of("abcdefghijklmnopqrstuvwxyz._012345678") != std::string::npos) {
        WUBAI_LOG_ERROR(WUBAI_LOG_ROOT()) << "Config invalid name: " << prefix << " : " <<node;
        return;
    }
...
void Config::LoadFromConfDir(const std::string& path, bool force) {
    ...
    for(auto& i : files) {
        ...
        try {
            ...
            LoadFromYaml(root);
            WUBAI_LOG_INFO(g_logger) << "LoadConfFile file=" << i << " successful";
        } catch(...) {
            WUBAI_LOG_ERROR(g_logger) << "LoadConfFile file=" << i << " failed";
        }
    }
}
